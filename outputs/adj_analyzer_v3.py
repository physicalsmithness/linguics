"""V3: remove possessives/ordinals/demonstratives from Class I;
they inflect similarly but belong to other topics.
Also tighten a few adverb-look-alikes."""
import json, re
from collections import defaultdict

# Load v2 code, then patch
import importlib.util
spec = importlib.util.spec_from_file_location('v2', 'outputs/adj_analyzer_v2.py')
v2 = importlib.util.module_from_spec(spec)
# We'll re-build lexicon here inline instead of importing to avoid re-running main
...
COMMON_NONNOUNS = {'e','o','ma','però','anche','non','più','meno','molto','poco','tanto','troppo','così','qui','là',
                   ...
                   'benché','sebbene','affinché','purché','qualora','poiché','giacché',
                   'talora','talvolta','forse','magari','proprio',
...
FEM_NOUN_HINTS = {'mano','moto','foto','radio','ala','arma','crisi','oasi','tesi',
                  ...
                  'stagione','domenica','settimana','giornata','vacanza',
                  'mattina','sera','notte',
                  }
...
def build_lexicon():
    ...
    CLASS_I = [
        ...
        # REMOVED: mio/tuo/suo/nostro/vostro (possessive topic)
        # REMOVED: primo/secondo/terzo/quarto/quinto/ultimo/ottimo (numeral/quantifier — arguable)
        # KEEP: ottimo — it IS a descriptive superlative adjective, not just a quantifier
        ('ottimo', {}),
        ('proprio_NO', {}),  # possessive
        ('sinistro', {}), ('destro', {}),
        ('rotondo', {}), ('quadrato', {}),
        ('vivo', {}), ('morto', {}), ('nato', {}),
        ('buono', {'special':'buono'}),
...
def main():
    ...
    for b, n in sorted(per_bucket.items(), key=lambda x:-x[1]):
        print(f'  {n:5d}  {b}')
    p['findings'] = findings
    with open('outputs/SEARCH_adjective_agreement_returned_v3.json','w') as f:
