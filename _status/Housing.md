...
queue:                     # derived from the name-grep 2026-07-17, judged live-vs-historical
  ...
  - push r17-r21 (pulse, level toggles, coverage: stripes->liquid layers + grey + groups + load-gate)   [external-action, Smith]
  ...
  - r21 row-height verdict: scaled rows (shipped, on trial) vs Smith's fixed-height-divided intent - one-line revert if ruled against   [decision, Smith]
