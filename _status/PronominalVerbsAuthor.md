...
queue:                     # name+class grep MINUS discharged, re-derived this turn
  - Architecture_PronominalVerbsAuthor_false_miss_packet v2       [Architecture's turn — verify + stamp 4 fixes]
  - Architecture_ALLAUTHORS_answer_leak_dispatch v5 (+v7 pointer) [Architecture's turn — verify + stamp pv_mech_sela_04; v6 roster re-listed me in error]
  - Architecture_PronominalVerbsAuthor_clitic_order_battery v1    [Architecture's turn — placement + cross-tree ingest]
  - Architecture_ALLAUTHORS_cue_notation... v-react              [Architecture's turn — tag my 5 multi_select rubric-cues as convention, no rework]
not_owed_verified:         # checked this turn, no action
  - item_shape_no_change_forms: not named; my cues are conjugated verbs (answer != cue); ci/ne immune per Rev 30
  - banned_sincerity_words: CLOSED, nothing owed by Linguics; 0 banned words in my explanations
  - Rev 33 (4 additions, all-authors): person field = set on NEW verb-formation items (none pending); common_errors optional; lr-exemption + item-shape n/a to my content
parked:
