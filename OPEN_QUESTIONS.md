...
- **Impersonal `fare` (weather), present tense — no home leaf.** "fa caldo / fa freddo" is a distinct A1/A2 skill (FARE not ESSERE for weather). Present tense has no bucket; `verb_form.imperfect.usage.age_time_weather` is the precedent. Proposed: a present sibling (owner: PresentUsage tree, or a cross-cutting impersonal-expressions leaf). Raised 2026-07-21 from Smith's live translation mark. Decision: mint + owner, or fold into present_indicative usage. (Not minted on spec.)

- **Grammar grading strategy - the deterministic norm() marker's structural limits (Smith, 2026-07-21: "I do not like how we grade... it cripples us").** norm() folds apostrophe/accent to space + substring matching -> the recurring bug family (apostrophe/accent ungradable typed, additive false-credit, anchoring). The AI marker does NOT share these (it reads the real glyphs, judges per-skill). Strategic call to make: where typed orthography matters, migrate grammar marking to the AI marker; keep the deterministic marker for instant/offline feedback + MCQ (index-scored, exact, no norm()). Decide the division of labour and whether AI marking expands beyond translation. Also: per-choice MCQ misconception tags are already ruled opt-in and should be UNIVERSAL on every MCQ - audit whether existing MCQs carry them.

- **Vocab valid-alternatives (arancia/arancione) = DETERMINISTIC data fix, not AI.** markVocab() already accepts an `alternatives` array (shows "Also accepted:"); the miss was a missing alternative + an ambiguous prompt. Fix in DATA (split orange fruit/colour, populate alternatives). AI-for-vocab NOT needed and costly on a high-volume strand; revisit only under the grammar-grading-strategy question if a genuinely open-ended vocab case appears.
