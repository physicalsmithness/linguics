...
**Ruling: run it centrally, discharge 26 of 29 topics at a stroke.** Waiting for 25 seats to each self-audit would have cost 25 wake cycles to find 5 items — and is exactly how 13 stayed unrun for months. `[discharged: all-authors except PronounAuthor, 2026-07-17, central chip audit]`. **This is the general shape for any `binds: all-authors` retrofit that is mechanically detectable: Architecture runs it once, stamps the class, routes the residue.** A per-seat self-audit is for judgement work only. [discharged: PresentUsageAuthor, 2026-07-20, batch_review v2 — crit 13 re-run on the 12 merged pu_ items, zero rule-naming hits] [discharged: PassatoAuthor, 2026-07-18, criteria_13_17 v1 — chip audit: no chip field, 10 prompt hits all output-form naming, 0 rewrites; crit 17: 103/103 self-glossed, clean in the 2026-07-17 survey] [discharged: PronounAuthor, 2026-07-20, criterion13_chip_audit v2 — 4 chips trimmed, essere_03 held for ruling; THE CENTRAL EXCEPTION IS NOW CLOSED: all 27 topics discharged] [discharged: ReportedSpeechAuthor, 2026-07-20, wake self-check — 8 bracketed cues all context clauses, 0 rule-naming]
...
**2026-07-19, later: Smith ruled B on the formation-trigger retrofit — per-author rework, not a central append.** Each of the [discharged: ConditionalFormationAuthor, 2026-07-20, trigger_retrofit v2 — 19/19 cue-named, 17 competitor guards word-anchored; verified] [discharged: FutureFormationAuthor, 2026-07-20, trigger_retrofit v3 — 45/45 name futuro, fut_fant_ already-forced excluded; vedere repaired Ti vedrò; verified] [discharged: PresentFormationAuthor, 2026-07-20, trigger_retrofit v2 — 7/7 frame-strengthened to present-time, flag honest; verified]
...
**2026-07-20, later: AccentAuthor and StressAuthor stood up** — dispatches written (DISPATCH_accent_drill.md, DISPATCH_stress.md), both deriving from DESIGN_accent_and_stress_drills.md. Taxonomy-first and data-first respectively; both adopt index-scored quick-fire shapes, the variant pipeline, and day-one pulse. Accent seat extends the existing orthography.accent.italian tree (error modes .missing/.added/.wrong_mark already registered). Boundary: written mark vs spoken prominence, Rev 27 cross-credit over fences.

## Desk cleared into the estate: migration run, pulse stamped, multi-select ruled, packets out (2026-07-21)

1. **The key migration is DONE: 1,936 keys renamed** `prompt_supplies_base_form` → `wrong_answer_is_form_error_only` across all 32 live grammar files, values preserved, legacy residual 0. (The remembered figure was 1,665; the corpus had grown under it — the command, not the number, was right.) Acceptance test per Housing r16: a clean deployed console. Housing's naming rationale is adopted as the estate standard: **item fields are named for the AUTHOR'S CLAIM, not the engine mechanism** — a mechanism name goes stale; the claim is what an author must affirm reading the item cold. ExistentialAuthor's early neg_01 unset verified and stamped.
2. **The shared pulse is LIVE and STAMPED** (real learner: Fletcher, Y13, session_start + 13 attempts in the linguics tab). Carried to EdTech as schema owner: propose a VISIBLE `answer` column estate-wide (Smith's first read of real rows was "it doesn't have what they answered"; it was buried in extra_json — r33 promotes it to the first key, but a teacher reading raw hits the same wall; physics will too).
3. **Multi-select qtype RULED, build commissioned**: scoring (right−wrong)/total-right floored at 0 (Housing's recommendation adopted — pays partial knowledge, zeroes select-everything); per-choice bucket/misconception tags SUPPORTED opt-in with an item-level default (Rev 27 diagnostics without author obligation).
4. **12 false-miss packets dispatched** (one per owning seat, pointing at the seat's itemised audit section, remedy palette + rails inline). Sincerity retrofit executed (16 fragments, zero banned words left outside exemptions) and the MetaProject thread closed with the canonical-copy ruling (Claude-drive Linguics canonical; OneDrive mirror decommissioning). [discharged: PronounAuthor, 2026-07-20, false_miss_packet v2 — 5/5 remediated, phrases/reframes verified on disk; first packet returned]
5. **Queued work order to cut, from the pulse verification**: misconception-tag rollout — only 221 of 4,944 guards estate-wide carry tags (tense_choice 128, imperativo 88, imperfect 5), so most wrong answers pulse with an honest blank. Rides MisconceptionAnalyst's 152 tag-lists + authors' open touches; not a new seat. [discharged: PluperfectAuthor, 2026-07-20, Phase-3 tagging — 88/88 guards tagged (34 wrong_auxiliary_tense / 48 participle_essere_subject / 6 participle_preceding_dop); first rollout seat; guard key is `misconception`, matching the live tense_choice shape]
6. Pulse-strip pointers absorbed: the inert STR micro-box is the stress drill's entry point (StressAuthor's dispatch notes it); Smith owes Housing one sentence on the 13/23-counts question and the order-leak repro (relayed in chat).

## Delta worked: usage axis COMPLETE, dual-credit ruled (b), the vocab fork's real cost found (2026-07-21)

**Condizionale usage accepted (23+19, self-merged after the author's own orphan-file catch — the merge convention is now the norm: one topic file, formation + usage together). With it, all five wave-2 usage branches are authored, wired and destubbed: the usage axis is COMPLETE.** Dual-credit ruled (b) against the author's (a): saturation is a coverage fact; Rev 27 credits evidence, and the future-anteriore and gerundio-passato parallels bind; six markpoints to add on the seat's next touch. **The vocabulary POS-fork ruling wrote itself once the code was read: 4-segment is canonical AND the housing already DROPS legacy 3-segment events as unattributable — so the 166 legacy vocab_help aspect ids across the estate (166 by grep — this entry briefly said ~640, a remembered guess written before the count printed; the correction stands here as its own small lesson) are silently losing their help-usage events today.** Central migration queued (board task). No vocab minting, ever: the vocabulary namespace is dynamic by design (root description + parser confirmed); the §2 forward-ref strict-reject doctrine applies to registered grammar trees only — recorded so no author repeats the demonstrative seat's (reasonable) worry. Also ratified: Ti vedrò; Future's usage-item person backfill; weights stay 0.5/0.5 on deceptive-gender co-credits (mastery is weight-independent; asymmetry buys nothing). 17 stamps applied this pass after spot-audits (one probe of mine was wrong, not the clerk: pluperfect guard tags live under the key `misconception`, the same shape tense_choice's 128 live tags use).

## Seven ruling clusters cleared (2026-07-21, second descent)

**Aspettare ingest (b)** — expand two seeds, full review, calibrate, then scale; multi-select seed rides the order thread; **transfer.dop_iop_confusion minted**. **Order battery**: `pronoun.discrimination.clitic_ordering` minted under a new minimal discrimination aggregate; **qtype-first, no MCQ forks** (the both-correct teaching is the instrument); essere_03 trimmed to the tense-name (Rev 26: trigger licensed, construction-shape is diagnostic). **Negation**: né…né addendum accepted; number-agreement STAYS with the correlative construction (a diagnostic lives with the construction that provokes it); a_meno_che split accepted pending replica; piacere co-credit declined on markability, not principle. **Pluperfect**: avere-for-essere guards COMMISSIONED (a classic real miss currently passing untagged); the mixed-gender item retagged to the finer id. **Lemma pilot**: recommendation (a) hybrid to Smith, live-feel gate stands; suppletives list-each-stem; the no-minting fact recorded estate-wide (dynamic vocabulary namespace). **Prisms**: four-key misconception-event schema ADOPTED (Housing thread cut); surface-lens layer adopted, analyst-authored, also the accent/stress drills' declared axis; **view==drill RATIFIED** (a prism IS a session filter); person spread already in flight via criterion-21 touches; comprehension arm parked. **ReportedSpeech** accepted in full — home stays future_to_past ("the boundary of a skill belongs with the skill"), deixis_05 stays suppressed. **Passive** pas_ven_03 ratified as surface. **Imperativo frozen-marker nulls ratified** (2sg shape is etymology, not tested person). Threads closed this descent: negation scope, reportedspeech disposition, passive compliance, imperativo backfill (+ earlier today: demonstrative live-round, vedere_ci, future trigger, condizionale usage pending its six markpoints).



## Verify-and-stamp sweep: draft-stamps applied (2026-07-21, Architecture)

Three deliveries verified satisfied on disk and stamped (spot-audited); threads closed:
- [discharged: Architecture_AdjectiveAuthor_buono_02_prompt_bug, 2026-07-21 - adj_buono_02 prompt now 'un ____ studente'; buon graded 0.7; buoni guarded]
- [discharged: Architecture_ComparisonAuthor_false_miss_packet, 2026-07-21 - cmp_eq_01 (quanto me), cmp_eq_03 (bello quanto), cmp_pmd_04 (oltre venti @0.9) on disk]
- [discharged: Architecture_ImperativoAuthor_discharges, 2026-07-21 - Rev-25 cue audit (18 cues, ranks 3-901) exempt/reglossed; crit-13 + tagging already stamped]

Also this sweep: 23 stale-close threads had their Next line flipped to CLOSED (rulings already on record; refs cited in each thread). Two 'ready' items were NOT auto-stamped and returned to the ruling pile: Architecture_PronounAuthor_brief_rev19_audit (dodge-notation scope; thread defers to Smith) and Architecture_Vocab_display_theme_grouping (theme-pin unpinned).


## Rev 29: criterion 20's gloss remedy inherits an ambiguity condition (2026-07-21, from AdverbAuthor + NegationAuthor, remedy from Smith)

Two seats hit the same collision independently. Rev 22/25 say 'gloss the target in English' when a cue is above level or reads as the answer — but where the gloss is a one-to-one translation of the single answer (only->solo, slowly->lentamente, well->bene), the gloss IS the answer and on a derivation item it destroys the item. **Ruling: gloss only where the English is ambiguous across the candidate set; else do not gloss.** Three remedies, precedence order: (i) widen the gloss until it spans the candidate set (Smith's adv_bb_07: 'He sings well / He's a good singer' -> {bene,buono}, which baits the exact miss — cheapest, keeps the instrument, converts giveaway to trap); (ii) change the instrument to MCQ (NegationAuthor neg_restr_01, v7-Q4 precedent); (iii) vocab_help the citation form (AdverbAuthor -mente). `binds: all-authors`; retrofit = the two seats' items, applied + stamped below; new items comply on authoring. Brief updated (Rev 29 + criterion 20 text). Third instance (Rev 23, 25, 29) of one criterion disarming another; the collision is the recurring bug.
[discharged: NegationAuthor, 2026-07-21, criterion20_retrofit v1 — 11 items glossed/standing per the level rule; neg_restr_01 -> MCQ; extension ruled as Rev 29; adv_bb_07 routed and confirmed fixed]
[discharged: AdverbAuthor, 2026-07-18, rev25_cue_level_retrofit v1 — 26 cues audited, 26 exempt, 3 vocab_help'd (raro/probabile/regolare); derivation-item remedy collision ruled as Rev 29]


## Batch acceptances: Connective, Indefinite, Existential rework, Noun (2026-07-21, Architecture)

Four delivered, replica-verified batches accepted; corpus mutations applied and re-parsed (manifest 31->33; misconceptions +2; connective +1 leaf).

**Connective (54 grammar + 22 translation) ACCEPTED.** Manifest +connective. Classes [all-seats, all-authors] ratified (originates markpoints/chips/guards -> Rev 24 originates-test). crit-13 sweep + Rev-25 cue-level compliance recorded (zero-hit at delivery). **connective.time.mentre_appena MINTED** (flat leaf under connective.time; conn_time_05/06 strict-rejected until now). In-batch calls upheld: ma-pero additive MCQ-only + unguarded (crit-18 fourth direction, index scoring carries it); uniform suppression on frame-forced recoverability (Rev 20 sixth class); poiche-casual graded 0.6 as the named common_miss (Rev 20(i)); crit-19 N/A. Glossary terms (connective, concessive, register, sequencer) accepted, merge queued with the glossary backlog. **PURPOSE-CONNECTIVE GAP (affinche/perche+subj) NOT ruled** - needs a leaf decision AND a mood-contract boundary with tense_choice; parked to Smith + OPEN_QUESTIONS to avoid re-running the phantom-stub duplication one level down.
[discharged: ConnectiveAuthor, 2026-07-18, batch_delivery v1 - 54+22 accepted, manifest wired, mentre_appena minted, classes + compliance recorded]

**Indefinite (61 grammar + 25 translation) ACCEPTED.** Manifest +indefinite. **nessun'idea BLESSED**: norm() folds the apostrophe (verified), so nessun'idea == nessun idea to the substring engine; ind_nn_05 accepts both at 1.0, teaches elision in the explanation, grades it in translation where the AI marker sees the glyph. **nessuna idea STAYS 1.0** (not 0.9): correct, on-pattern Italian (reaches for nessuno + feminine agreement), not a dodge - declined to invent a prescriptivism. candidate_forms: seat applied (A); binds on the Article A/B/C ruling (one-pass retrofit if (B)). **ASK 1 ADOPTED (guard-verdict gate)**: add `assert mark(item,guard).outcome=="miss"` to the central anchoring gate - a containment check cannot see a positive nested in a guard (live false credit) or a guard nested in a positive (dead catch); three independent instances (Pluperfect 5-week bug, WordFormation deleted gender test, ind_chi_01) prove the class. Needs the marker in the gate; tasked.
[complied: IndefiniteAuthor, 2026-07-18, batch_delivery v1 - crit 13/17/18/19/20/Rev-25 clean, replica 62 markpoints 0 findings after 2 self-fixes]

**Existential rework (16 items) ACCEPTED.** **transfer.partitive_for_bare_noun MINTED** (guard population = 5 bare-noun traps; Smith's "will they over-translate 'any'?" test now markable). A2/B1 esserci cues STAY (at-level by A2; the A1 gate was the exposure). Rev-27 cross-credit KEPT on the 4 co-credited traps: the atomic widened blank means a number error false-MISSES the co-bucket, never false-CREDITS; mastery-on-both on correct answers is worth the known trade.
[discharged: ExistentialAuthor, 2026-07-18, cue_level_and_additive_credit v2 - 16 items, partitive minted, A2 cues stand, cross-credit kept]

**Noun residuals ACCEPTED.** **orthography.io_plural_i_count MINTED** (figlii/zii i-count misses had no home; same shape as the cia_gia gap). candidate_forms on 4 meaning-pair items + 9 spelling-leaf misconception tags stamped. crit-25/20/19 no-ops recorded; crit-21 N/A. Two further proposed ids (English-s transfer; noun lexical-gender) PARKED for the Analyst's registry harvest (no mint on spec - habitual_frame_mismatch precedent).
[discharged: NounAuthor, 2026-07-21, batch_disposition v2 - candidate_forms + tags stamped, io_plural_i_count minted, crit-25 no-op stamped]


## Rulings pass 2: residue GO, candidate_forms, cio, congiuntivo labels, TenseChoice compliance, pari (2026-07-21, Architecture)

**RESIDUE ENGINE - GO.** Housing's survey is clean (1,839 accepted answers, 0 flips under both rules; instrument validated on synthetic cases). Approved to ship. Rulings: (i) ALL-matches subtraction (order-proof); (ii) whole-prompt token licensing (produced the clean zero); scope single-markpoint only (191 multi-markpoint keep today's semantics, detected by markpoints.length===1); wire the soft second-chance banner ("your extra word: X - is that part of your answer?") so a spontaneous variant flips to a gentle prompt, not a hard miss. Retires criterion 18's fourth direction from every author's checklist; makes Smith's "will they over-translate 'any'?" test run estate-wide by default. Parked whole-answer match_at:"exact" superseded (it trades a false credit for a false miss). Next: Housing builds in grammar_engine.js.

**ARTICLE candidate_forms - (A).** MCQ discrimination items don't need the fields (buttons render the set); add only to the suppressed art_disc_05. WordFormation verdict-MCQ precedent. Confirms IndefiniteAuthor ask-4 (no MCQ retrofit). Usage-omission items (not .discrimination) not bound by crit 16.
[discharged: ArticleAuthor, 2026-07-18, discrimination_candidate_forms v1 - ruled (A), art_disc_05 only]

**DEMONSTRATIVE cio - (a), generalised to Rev 30.** Confirm the acting default: gloss + pin formal register, cio target 1.0, quello che everyday 0.9 + steering note (register per crit 11; Rev 20(i) allows the 0.9). (b) would lose the cio drill. Generalised as Rev 30 (invariable words don't reach the citation-form exemption). Three discharges stamped; classes [all-seats, all-authors] ratified.
[discharged: DemonstrativeAuthor, 2026-07-15, dispatch_outputs v4 - 5 Rev-19 flags flipped to show, candidate_forms added, 0 suppressed]
[discharged: DemonstrativeAuthor, 2026-07-18, class_retrofit_audits v1 - crit-13 44 items 0 rule-naming; Rev-25 3 cues at/below level; 0 rewrites]

**CONGIUNTIVO compound labels - relabelled.** The two compound leaves embedded the worked answer in the pre-answer breadcrumb (the 2026-07-12 terse-label ruling missed the congiuntivo tree, scaffolded later). Now "Congiuntivo passato (compound)" / "Congiuntivo trapassato (compound)"; worked examples stay in description. Estate grep confirms no other bucket label embeds a worked answer - class closed. Next: CongiuntivoFormationAuthor re-audits the 16 wave-3 suppressions, reports which flip to visible.

**TENSECHOICE compliance - 3 stamps + Q1 (Rev 31) + Q2 ratified.** The crit-13/19/20 discharges are no-ops BY CONSTRUCTION, stamped. Q1: criteria 13 and 20 gain a supplied-choice exemption mirroring 19's Rev 21(i) -> Rev 31, so no future selection seat re-derives the zero. Q2: compliance register gains `n/a (by construction)` as a state distinct from `discharged` - ratified (fourth label-overload instance). Routed finding (9 redirects "still stub") is STALE: derived from disk, none carries stub:true; they carry deprecated_authoring_target + active:false + superseded_by = the "own flag" asked for. If the denominator still counts active:false nodes, that is a Housing denominator question (routed).
[discharged: TenseChoiceAuthor, 2026-07-18, criterion_compliance v1 - crit 13/19/20 all n/a-by-construction (supplied-choice), evidence reproduced from disk]

**PARI family (Adjective) - point-2 (a).** Invariability is the load-bearing B2 skill; lemma-choice is productive MISDIRECTION, not "the real skill." Rev-19 candidate frame stays; diagnostic + must_not_include target the inflection attempts (pare/parie/paria). Design principle from Smith, recorded: misdirection is a legitimate item-design tool; a rewrite that over-signposts the tested skill loses its teeth - where two skills stack, don't name the decoy as "the real skill" in scaffolding (candidate for the brief House-techniques section). Next: AdjectiveAuthor drafts 12-15 items.

## Smith's mid-turn ratifications (2026-07-21)

- **Volume dispatch: GO** — "increase the volume, do the big jobs." DISPATCH_volume_formation.md issued to all ten verb-formation authors: more basic high-frequency items for fluency reps, `person` on every item, the three regular classes kept separate, standard quality gates attached, depth over exotica. The coverage-growth lever.
- **Rich event-descriptor schema: YES, "everything retrievable, via a JOIN KEY."** Supersedes BOTH the four-key model and any fat denormalised event: the event carries a join key (attempt/item id) to the item's descriptor set (person, strand, preposition, conjugation-class, tense, ...); the stats layer joins and groups by any descriptor. Person and strand are just two of many. Build the join, not the wide row. Routed to Housing.
- **Surface lens: analyst authors FOR REVIEW.** Smith wants to see it before final okay; the analyst proceeds, Smith reviews the output before it goes live.
- **Coverage matrix: Smith ruled directly with Housing — keep BOTH views, make the toggle findable.** Architecture side of the coverage_matrix thread closes; scoring-formula discussion stays Smith<->Housing.
- **Purpose connectives (affinche/perche + subjunctive): "generally both."** Resolved via Rev 27: author a connective home (the connective IS the mood trigger) that CROSS-CREDITS tense_choice.indicative_vs_subjunctive on the same answer. One item, two markpoints, both buckets record full correctness — no duplication, the phantom-stub worry dissolves. Boundary: connective owns the connective choice, tense_choice owns the indicative-vs-subjunctive contrast. Routed to ConnectiveAuthor.
- **"Drill in/out" is three different things — the word is the bug (again).** (1) drilling to ANALYSIS = a page navigation; (2) on the right-hand stats aside, "drill in/out" = APPLY / REMOVE A FILTER; (3) on the coverage page, a click should drill INTO analysis (forward), NOT take you back. Disambiguation routed to Housing; stop using one phrase for both the filter and the navigation.


## CEFR-harvest fill decisions (2026-07-21, from Smith)

- **Periodo ipotetico = INTEGRATED LEAF** (not its own topic). Minted `tense_choice.periodo_ipotetico`; one item cross-credits condizionale/congiuntivo/counterfactual buckets per Rev 27 (no duplication). Dispatched.
- **Interrogatives = new topic**, minted (root + chi / che_cosa / quale / quanto / adverbs / discrimination), manifest wired. Dispatched.
- **Formal vs informal address (mi scusi / scusami): ADOPT** as a register usage item. **Politeness via the conditional (Vorrei / Potresti): ADOPT.** Both fold into existing seats (register -> imperativo/register area; politeness -> condizionale usage), not new chats.
- **Error-identification item shape (Transparent): ADOPT** — a full sentence with one wrong word, options are words FROM the sentence, pick the error; recognition, index-scored; natural home for the over-reg distractors. Route the qtype to Housing + note in the brief.
- **Choose-the-reaction (the 'wallet stolen' item): DECLINED** — Smith: "that's really just reading comprehension," which stays out of scope.


## AccentAuthor placement-class taxonomy RATIFIED (2026-07-21)

Six placement classes ratified (meaning_pair, tense_bearing, che_compound, lexical_final, no_accent, apostrophe_not_accent) + the placement aggregate. The derivation (thread §7 — an accent records stress the reader cannot predict; monosyllable tonic/atonic twins vs polysyllable fixed-final-stress, with no_accent and apostrophe as the system's edges) is sound and corpus-exhaustive (595 accented types surveyed, none double-homed). **§3 ruled (a): classes MINTED AS LEAVES** under `orthography.accent.italian.placement` (7 nodes) — per-class mastery rides the coverage matrix + scoped practice, and the double event (error-mode leaf + placement leaf) is a Rev-27 feature not a cost. **pron_effect (stress_shift | vowel_quality | none) ratified as an ATTRIBUTE**, not a leaf — it cross-cuts placement and is the cross-credit seam with StressAuthor. **§4 apostrophe_not_accent kept under .accent.placement** (the learner meets it in the accent drill; index-scoring finally grades it past norm()'s apostrophe-fold), recorded as a truncation apostrophe whose formal home is orthography.apostrophe. **Event contract ratified** (accent_type x placement_class x outcome -> .missing/.added/.wrong_mark; insertions attribute to the type TYPED; wrong_kind carries both chars so the acute<->grave direction is recoverable) — routed to Housing with the stress report axes. **§5 wrong-kind verdict wording is SMITH's** (he holds the IB subject reports, §9); AccentAuthor's recommended tiering relayed for his call: wrong-kind = mildest tier (name-never-dock, per the live accent policy), omission on tense_bearing/meaning_pair = the serious tier. AccentAuthor cleared to author Deliverable 2.


## Cr17 marker-replica re-run + estate-wide guard-verdict run (2026-07-21, Architecture)

Ported grammar_engine.js + norm.js into a Node/vm harness (/tmp). **Cr17 5 topics: 2009 guards, 0 false-credit; 553 single-markpoint targets, 0 not-full** — the five CANNOT-VERIFY 'zero marking delta' threads are now VERIFIED and closed. **Estate-wide (31 topics, 2300 items): 5365 guards tested, 16 scored full credit, 2000 single-markpoint targets all full.** Triage of the 16:
- **12 are MCQ items** (poss_fam_01/02/07/08/13/14, poss_pred_01..05, op_comb_joined_01) — scored by choice-index, so the substring guard is INERT; no live bug. Confirms PossessiveAuthor's mitigation (family "drop the article" moved to MCQ because match_at word can't reject a leading article). **Gate refinement: the guard-verdict gate must SKIP mcq items** (test typed items only) or it false-positives on every documented-but-inert MCQ guard.
- **2 are genuine LIVE false-credit** on typed "short" items, both clitic-ORDER: `op_comb_order_01` (bare any_phrases "lo"/"Lo" credit the wrong-order "lo mi dai") and `op_comb_glielo_11` (fragment "darglielo" credits the wrong-order "darglielo voglio"). The **residue engine does NOT fix these** — the extra/reordered words are prompt-supplied, so residue clears to empty and the fragment still matches. Word ORDER is the error, which substring marking cannot see. Routed to PronounAuthor for the order-battery instrument (index-scored / order-aware, per the clitic_ordering ruling).

## Phantom translation bucket-ids resolved (2026-07-21, Architecture)

Derived, not the remembered "27": **7** phantom bucket refs, all in `optional_buckets` (secondary cross-credit tags, so no item's required coverage was broken). **5 remapped to real leaves** — imperfetto + imperfect_indicative -> verb_form.imperfect; subjunctive.present -> verb_form.congiuntivo.formation.presente; verb.piacere -> piacere.form.agreement; tense_for_ongoing_duration -> verb_form.present_indicative. **2 dropped as micro-gaps** with no grammar home: `exclamative.che` (Che bel libro!) and `time_expression` (Sono le sei e mezza / telling the time). Re-scan clean (0 phantom). Micro-gaps noted in OPEN_QUESTIONS.

## Glossary merge (2026-07-21, Architecture)

Merged the seat-proposed glossary backlog into glossary.json: **+24 terms (151 -> 175)**, seat-authored short/long/aliases, deduped against existing terms + aliases. Added: connective, concessive, sequencer, temporal subordinate clause, passive voice, dynamic passive, proprio, si-impersonal, stative verb, conditional perfect, present subjunctive, suppletive, theme vowel, syncopation, assimilation, future perfect, progressive, compound gerund, prospective reading, polite-present, strong stem, trapassato remoto, stem, diphthong. ~6 candidates skipped as already covered (existing term or alias). **'impure s' (article) skipped — its suggestion carried no definition; flagged for ArticleAuthor to supply one.** NOTE: the claimed estate-wide gap "subjunctive/gerund/indicative absent" was STALE — all three (plus congiuntivo/condizionale) are already in the glossary. Derive over memory again.


## vocab_help legacy-id migration ({}) — safe bulk done, residue flagged (2026-07-21, Architecture)
Derived 221 legacy 3-segment vocab_help/markpoint ids (missing POS; the housing drops them as unattributable). **Migrated 133 safely** to the 4-segment POS shape: 127 where the lemma has a UNIQUE POS in vocabulary_it_frequency.json, + 6 reflexive infinitives (-rsi, morphologically certain verbs). Left `-are/-ere/-ire` unforced (noun exceptions like *mare*). **88 residue** written to `incoming drafts/vocab_help_legacy_residue.json` — two classes: multi-POS lemmas (need item context to pick POS) and not-in-freq (inflected forms used as the vh lemma, amici->amico, belle->bello, or proper nouns; need lemmatization). Not auto-guessed — a wrong POS pollutes the wrong bucket, worse than the recoverable drop. Residue is a careful context/lemmatization follow-up.


## Volume-wave acceptance batch + free-choice seam (2026-07-21, Architecture)

The volume dispatch LANDED: corpus 2426 grammar (+265 since seat handover). **18 returned deliveries ACCEPTED + stamped** (volume waves Conditional/Congiuntivo/Future/Imperfect/Passato/PassatoRemoto/Present, formation-trigger + false-miss + prompt-bug retrofits, Possessive suo fields, Pronoun rev19 dark-leaves, Vocab theme prune). Verified by the estate-wide guard-verdict harness: **0 new false-credit across 2426 items, all 2126 single-markpoint targets full** — the new items introduced no marking regressions. Two caveats recorded on their threads: (i) op_comb_order_01 is NOT clean despite PronounAuthor's rev19 claim (known clitic-order bug, routed to the order battery); (ii) the central person-migration parser mis-read leaf tokens like '1sg' as the person on ~12 congiuntivo items — parser fix + estate-wide person re-audit tasked.

**Free-choice-subjunctive seam RULED (consistent with purpose-connective):** minted `tense_choice.indicative_vs_subjunctive.free_choice_triggers` as the mood cross-credit target; the WORDS stay in indefinite.free_choice (IndefiniteAuthor authors), an item cross-credits both per Rev 27. Resolves the trigger that was owned by nobody. IndefiniteAuthor unblocked.

## Four Smith rulings + design-overhaul reconciliation (2026-07-21)

- **ACCENT DRILL NOW DOCKS (reverses never-dock).** Smith: "let's dock, whatever the answer is." Wrong-KIND (wrong direction e-grave/e-acute) = **0.8** (dock 0.2 — "let's not dock them very much... give them 0.8 for getting it the wrong way around"); OMISSION (no accent) ~= **0.5** (halfway, provisional — exact + class-severity refinement TBD, "not a pressing matter"); correct 1.0. Direction ("which way round") surfaced in the report. Routed to AccentAuthor (drill scoring) + the design reconciliation (accent bar). Supersedes the 0.9-flat/never-dock live policy.
- **Rev 32 — cue tense-naming.** Both EN+IT where the tense matters and isn't obvious; imperfetto either (obvious); subjunctive/congiuntivo everywhere (always). Agent audits compliance (spawned).
- **Lemma-retrieval pilot APPROVED.** Smith reviewed the 3 taster items AND the next 12 — all fine. PresentFormationAuthor proceeds to author the 12 + convert per the standing pattern. (Decision 5 resolved.)
- **Pulse-strip "13/23 counts" (decision 6) NOT resolved:** Smith has flagged it repeatedly and reports the counts still "get chopped off" — a display/truncation fault, not a spec gap. Relayed to Housing to fix the truncation; Smith will state the exact expected behaviour on the live-look.
- **Claude Design overhaul reconciled** against the 7 in-flight specs: no hard conflicts; the design DELIVERS coverage-canvas / analysis-surface / hover-fix / drill-navigation, and must ABSORB residue engine + error-ID qtype + accent-docking + the STR/ACC report spec. Full map: inter_chat/Architecture_Housing_design_overhaul_reconciliation.md.

## Rev 32 CORRECTED + PassatoRemoto calls ruled (2026-07-21)

**Rev 32 was over-broadened by Architecture and is unwound.** Smith's actual rule: a both-languages CONSISTENCY rule — where a cue ALREADY names a tense in one language and the translation is non-trivial, give both; let off trivial (present) and cognates (futuro/condizionale/gerundio/imperfetto, confirmed obvious). It is NOT force-naming, and it must NEVER name the mood on a subjunctive USAGE/DISCRIMINATION item ("we wouldn't expect subjunctive to name itself if it's a test of when to use it") — that hands over the answer. **Corrected retrofit worklist ≈ 118** (passato_prossimo 54 + passato_remoto 64 formation cues named Italian-only, non-trivial → add English). DROPPED from the earlier audit: the 136 congiuntivo "name the mood everywhere" (backwards — was my error) and the 44 force-name-no-tense. The 8 imperfect DISCRIMINATION items are a SEPARATE question (do they leak by naming the tense at all?), not Rev 32.

**PassatoRemoto grading/level calls — Smith delegated 2 & 3 to Architecture ("you call"); all four ruled (a) status-quo:** (1) dare diede/dette equal at 1.0; (2) -ei/-etti equal at 1.0 (both standard modern Italian — no house penalty); (3) hold all C1, recognition belongs to the deferred usage dispatch not a B2 down-tag of 5 production items; (4) archaic fé/dié left out (production items shouldn't invite them). No item changes needed — the batch stands as delivered; thread closes.

## "Mad use of use": the cue-notation class is a RENDERER fix (2026-07-21, Smith live catch)

Smith hit "use: usually" on a present item (pres_lr_bere_3pl_01, whose gloss ends "(usually)"). Tight audit: 9 clear cases (8 present lemma-pilot + adv_prim_04), all the same shape — a trailing English/aspect parenthetical that the renderer turns into a "Use X" chip. **Root cause is the renderer, not the data:** "Use X" is right for a citation-form lemma cue `(bere)` and wrong for an English gloss `(early)` or an aspect trigger `(usually)`. **Ruling: the "Use X" chip fires ONLY on a single Italian lemma; English glosses render as meaning, aspect/temporal words are not chipped at all** (routed to Housing, Architecture_Housing_cue_notation_renderer). This resolves the whole cue-notation retrofit (~138 "Use <english>" items, task 7) systemically — no per-item retrofit of the meaning-gloss cases. Data cleanup that remains: the 12 lemma-pilot prompts (trailing aspect parens + alzarsi's "(leave the bed)" steer), routed to PresentFormationAuthor to reword to plain-prose glosses. adv_prim_04's "(often)" is a meaning cue that the renderer fix handles.


## Passato prossimo agreement is over-split by orthographic ending; the ending is a BAND, not a leaf (2026-07-21, Smith's live catch)

Smith: "third row of pass pross is all agreements... doesn't need to be split per orthographic difference." Confirmed from disk: `participle_agreement.with_essere` is 4 separate leaves by ending — masculine_singular(-o) / feminine_singular(-a) / masculine_plural(-i) / feminine_plural(-e) — and `with_avere` splits the same way across preceding_dop_lo/la/li/le. **Ruling: that is over-granularity — agreeing the participle is ONE skill; the -o/-a/-i/-e ending is the gender-number of the subject/DOP, a BAND (like the person barcode), not a distinct leaf.** Consolidate: with_essere.{masc_sg,fem_sg,masc_pl,fem_pl} -> one `with_essere` leaf + a 4-cell gender-number band; with_avere.preceding_dop_{lo,la,li,le} -> one `preceding_dop` leaf + the same band. **Keep the genuinely-distinct sub-rules** as their own leaves: mixed_gender_default (mixed->masc), with_avere.no_preceding_dop (invariable), preceding_partitive_ne, preceding_dop_mi_ti_ci_vi (optional). Net: 8 ending-split leaves -> 2 banded leaves. This is the paradigm-band principle (already ratified for verb-formation person) applied to agreement; folded into the paradigm-bands migration (task 11). Re-point the ~12 items + supersede the 8 leaves; route the item re-point to PassatoAuthor.

**Also (same catch):** the coverage HOVER being unreadable is the known hover-text-columns problem (task 25) — the Claude Design overhaul fixes it (labelled person/gender bands + real column headers + "N of M questions attempted" tiles). Consolidating the agreement leaves also makes the pp coverage rows legible (fewer, clearer rows instead of 8 ending-split ones). And `pp_aux_person_02`'s "Use comprare" is the cue-renderer bug (context cue mis-chipped as operand), extension routed to Housing.

## Translation "harsh 0/4" is the offline STUB faking a verdict (2026-07-21, Smith live catch)
"I haven't read that book yet." -> "Non ho gia letto quello libro." scored 0/4 with "Negation: Wrong". Diagnosis: the OFFLINE STUB ("Real AI marker would say what" = its placeholder), not real marking. It fabricates a confident "Negation: Wrong" on a CORRECT negation (non before aux) by mock-marking a required_bucket, and gives 0/4 against its own "50% overlap". Real errors are only gia->ancora and quello->quel (negation + ho letto correct) -> a real AI mark is ~2/4. Fix routed (Architecture_Housing_translation_stub_honest): the stub must degrade honestly (label offline/approximate, no fabricated per-markpoint verdicts); connect the AI worker for real marking. NB the stub's code isn't in committed translation_marker.js or the mockups -> deployed build is ahead of committed source (recurring gap).

## Translation items are single-topic-authored; marking must go cross-topic (2026-07-21, Smith's catch)
"Some authors are just writing the mark scheme for their own part." Confirmed: 640/870 translation items (74%) have ONE required_bucket — filed per topic, authored by that topic's seat tagging only their slice. A cross-topic sentence ("...was really the worst" = article+prep+demonstrative+verb+adverb+superlative) marks against one bucket -> under-credits (Rev 27) + under-populates coverage. **Ruling: a translation credits every bucket it evidences; required_buckets are a FLOOR, not the ceiling. The scalable fix is the AI marker tagging cross-topic (bounded by the tree), NOT a 640-item retrofit.** Routed to Housing/Code (Architecture_Housing_translation_crosstopic_marking). The translation-marking contract is CENTRAL (architecture), not per-topic. Sharpens the README's flagged translation-pipeline rethink.

## Translation marking: dynamic per-answer denominator + amber tier; vocab picker is a filter not a navigation (2026-07-21, from Smith live)

**Marking score is X / N where N = the number of skills the marker surfaces for that answer** - not a fixed scale (corrects an earlier /5 draft of mine). This is just the v2 contract's `overall` = aggregate of the per-markpoint verdicts: the worker finds as many skills/misconceptions as it can, and the denominator is however many it returns (5, 7, 3, whatever the answer contains). correct = 1, partial (amber) = partial credit (weight is a dial), wrong = 0. The skill-by-skill feedback panel is ADOPTED as the translation feedback layout: score header `X / N` + credit meter; learner sentence coloured token-by-token (green correct / amber understood-not-idiomatic / red to-fix); a "what you got right" green-chip row crediting every bucket the answer evidences (required_buckets = floor, made visible); a red card for the tested error and an amber card for soft errors. **Amber = the v2 `partial` outcome made first-class** (Smith: nella centrocitta -> in centro is amber, not red). Offline stays honest per the stub ruling. Mockup: mockups/translation_feedback_panel.html. Routed to Housing at Architecture_Housing_translation_crosstopic_marking v4.

**Vocab category picker: in-place multi-select filter, not a forward navigation.** The pick-a-category-and-drill-into-a-new-screen model is REJECTED (no back button, cannot select a second, loses the overview) - the same view==drill / "a prism is a session filter" ruling. Category picks apply in place as removable chips, multi-select, overview preserved. Group by THEME (food, colours, ...); replace per-gender selectors with a single gender group (+ optional "complete irregulars"), because filtering to one gender leaks the answer. Plus a render-perf + text-squashing bug on the vocab screen (flex-column overflow family). Routed URGENTLY to Housing at Architecture_Housing_vocab_category_picker v1.

## Marker under-fires vocab on production (en_it); dynamic-vocab exception ruled (2026-07-21, from Smith live)

On en_it (production) translation items the worker only vocab-credited the injected help word (aprire); produced content words (finestra, caldo, qui, dentro) fired nothing, because the worker's VOCABULARY section is it_en-only and the "never fire a bucket outside bucket_context" guard blocks un-injected lemmas. Ruling: the vocabulary namespace is DYNAMIC, so vocab is the one sanctioned exception to that guard - on en_it the marker fires vocabulary.it.<lemma>.translation (hit/miss) for every produced CONTENT lemma (nouns/verbs/adjectives/adverbs/lexical locatives; not articles or grammatical particles). Grammar markpoints keep the guard. This populates the vocab strand from production and raises the per-answer skill count (X/N surfacing, v4/v5). Routed to Housing/worker at Architecture_Housing_translation_crosstopic_marking v6. Separately: impersonal FARE for weather (fa caldo) has no present-tense home (only imperfect.usage.age_time_weather); candidate present sibling parked to OPEN_QUESTIONS, no mint on spec.

## PROPOSED (awaiting Smith): categorise the feedback panel, collapse-by-default, drop the 136-fence (2026-07-21)

Design pass on the translation feedback panel (routed as crosstopic v7, PROPOSAL - not built until confirmed). (1) Credited skills split into NAMED categories with a colour each: Grammar/Vocabulary/Gender/Accent = blue/green/teal/purple, amber+red reserved for problem state. (2) Per-skill explanation collapsed by default, click to open, AUTO-open only amber/red (stops the page careering; native <details>). (3) Drop the 136-node bucket fence: free detection on OUTPUT (categorised, shown to the learner), with a reconciliation layer mapping detections to real leaf ids for coverage and logging unmapped detections as tree-gap candidates (also shrinks the marker prompt). OPEN CHOICE for Smith: colour = category with amber/red override (mockup) vs colour = correctness with category by grouping only.

## Feedback panel: correctness-dominant colour, Spelling category, reconciliation-is-code (2026-07-21, from Smith)

Resolves the v7 open colour choice. COLOUR = CORRECTNESS dominant (green right / amber slip / red wrong), whole-sentence token view restored at the top, red reduced to a thin mark not a block (a category-dominant palette read as a wall of red). Skill CATEGORY shown as a subtle muted dot+label only. NEW category SPELLING: the AI names the error class (missing/added double letter, single-for-double, transposition) - unhand-codeable, easy for the AI; pairs with Accent as orthography; worker emits a free `class` label, normalise later if useful. Marker RECONCILIATION (mapping free detections to real bucket ids once the 136-fence drops) is a DETERMINISTIC CODE step owned by Housing/Code, NOT a second AI; unmapped detections show to the learner + log as tree-gap candidates. Routed as crosstopic v8. Still open for Smith: final bless + spelling free-label vs enumerated; the drop-fence build tradeoff (v7).
...
## Gender/plural drill: the MCQ options ARE the misconception-distractors (2026-07-21, Architecture, for Housing)

Housing was waiting on the MCQ option sets. They are the per-class misconceptions realised as distractors (each tagged -> diagnostic). Primary format: singular-with-article -> pick the correct plural phrase (tests gender via the article + plural via the form in one). Housing GENERATES options from the vocab entry (gender, plural) + per-class distractor rules (regularise, english_s, no-change, wrong-article-from-ending, added-ending, over-reg braccio->bracci). Spec on Architecture_Housing_gender_plural_drill v2; distractor misconception ids -> MisconceptionAnalyst (gender_plural family); class->rule mapping -> NounAuthor.

## Gender driller REDESIGNED: uniform fixed-option classifier, bare noun, leak-free (2026-07-21, Smith)

Supersedes the per-class-question design. ONE question every time: show the BARE noun, learner picks its gender-behaviour class from a FIXED list (1 masculine, 2 feminine, 3 either-gender il/la giornalista, 4 depends-on-meaning il fine/la fine, 5 m-sg/f-pl braccio->braccia, 6 f-sg/m-pl eco->echi [rare], 7 two-plurals-different-gender braccio->bracci/braccia [advanced]). Same options = low cognition; bare lemma leaks nothing. Correct class DERIVED from the vocab entry (gender+plural); index-scored; wrong pick = the misclassification diagnostic. Narrow filter strips the obvious m/f. Scope = gender only (plural-form spelling is SpellingAuthor's). It's the driller but gender also cross-credits from agreement/translation items. Routed Architecture_Housing_gender_plural_drill v3.

## Existential item scores a tense-choice miss as formation (2026-07-21, Smith live) — another formation-supplies-trigger instance

op_cil_exist_03 (pronoun.ci_locative.existential): "Ieri ___ molta gente (there were many people)" failed c'era (a valid IMPERFECT existential) because it accepts only c'è stata (PP). Two faults: prompt tense-ambiguous (c'era valid/natural), and the miss is tense-CHOICE not existential-FORMATION (c'era is correctly formed). Instance of DECISIONS 2026-07-19. Routed to PronounAuthor: supply the tense trigger (force PP) or accept c'era; audit the ci_locative.existential set. Boundary flagged (pronoun.ci_locative.existential vs existential topic; tense-choice is tense_choice's). Inert guard noted: must_not "ci era" doesn't catch elided "c'era" (norm folds the apostrophe).

## Pass-1 extension RATIFIED: registry 81 -> 99, derivation family minted (2026-07-27, Architecture; Analyst rulings 1-4)

Ratified misconception_suggestions_pass1_extension.json (evidence-backed, spot-checked). MINTED 18 specifics + the derivation family; BROADENED 3 (agreement.prenominal_allomorph -> all sound-conditioned prenominal allomorphs across 5 surfaces; transfer family -> L1/prior-language not English-only; regularisation.invariable_adjective -> invariable WORDS). Highlights: **agreement.subject_verb_finite** (the big cross-kind - piacere/existential/si-passivante/volerci/indefinites; resolves the auxiliary_person question - auxiliary_person stays auxiliary-scoped, this is the main-verb sibling); **paradigm_confusion.{preposition_choice (123 guards - the largest uncovered surface), preposition_verb_governed, relative_choice, connective_choice, adverb_vs_adjective}** (paradigm_confusion already adopted); regularisation.{irregular_plural, irregular_comparative, noun_class_plural}; transfer.foreign_s_plural; **agreement.noun_gender_lexical = the gender-driller's "gender-from-ending" misclassification id (UNIFIED - one id, both instruments)**; discrimination.{no_backshift_over_extension, deixis_not_shifted}; word_order.{negation_elements, adverb_position}; derivation.{mente_formation, alterato_misuse, prefix_misuse}. Unlocks the 82 pending tag-lists. Comprehension/reading misses stay DEFERRED (reading-arm question parked, not lost). Analyst's "audit all 17 family descriptions for named-but-unminted cases" ADOPTED (commissioned).

Rulings 2-4: (2) v12's four direction-merged tense_choice ids stay MERGED - the confusion pair is the unit, direction recoverable, don't quadruple cells; no re-tag. (3) v13: courtesy-caps gap -> SpellingAuthor (orthography.spelling.capitalization); clitic-joined-as-one-word -> MINT (named-but-unminted in clitic_cluster; Analyst proposes the id); clitic_gender_number person-broadening APPROVED; $menu_rule tagging-guidance approved as proposed. (4) Merge pattern CONFIRMED: one combined misconception_tag_lists.json keyed by topic.

## Volume-wave acceptances: PassatoRemoto + Passive (2026-07-21, Architecture, task 2)

Two returned volume waves verified on disk + accepted.
**PassatoRemoto** (39->68): regulars-at-single-coverage RATIFIED - dispatch INTENT (fluency where needed, depth over exotica) over grid arithmetic; passato remoto regulars are low-frequency, and 2-for-green is a display floor reached by repeat attempts, not a per-cell authoring target. No wave 2.
**Passive** (37->45): **person convention RATIFIED as the ESTATE STANDARD** - `person` = the PRODUCED person (where the learner conjugates the finite verb), `null` where the answer is a preposition / MCQ index / English recognition; NOT the incidental clause person. This binds the task-5 person re-audit + parser fix (which must tag produced person only). PassiveAuthor stamped into the wave.
[discharged: PassatoRemotoAuthor, 2026-07-21, volume_wave v1 - 68 grammar verified; regulars-at-single-coverage RATIFIED (dispatch intent over grid arithmetic; 2-for-green is a display floor, not a per-cell authoring target); no wave 2]
[discharged: PassiveAuthor, 2026-07-21, volume_person_bands - 45 grammar verified; person convention RATIFIED as estate standard (produced person, not incidental; null where answer is prep/MCQ/English); stamped into the wave]

## Possessive edge-patterns ACCEPTED; 3 stubs cleared (2026-07-21, Architecture, task 3)
possessive.{predicate (e mio), postposed (casa mia), proprio}: all three ratified leaves now authored (grammar 63 incl. predicate 5MCQ+3t, postposed 4MCQ+3t, proprio 4short+2MCQ+3t), verified cited on disk; stub flags cleared on data/buckets/possessive.json. MCQ correct for predicate/postposed (article-absence + word-order aren't gradable typed); proprio short items guard il suo/i suoi as the named WRONG miss (Rev 20(i)). Sibling threads already closed in the sweep. Possessive seat COMPLETE.
[discharged: PossessiveAuthor, 2026-07-21, edge_patterns v3 - 3 leaves authored+cited, stubs cleared, batch accepted]

## Task 4: periodo ipotetico + Pluperfect tagging + Pluperfect cue-language RULED (2026-07-21, Architecture)

**periodo ipotetico ACCEPTED** (TenseChoice, 18 grammar MCQ + 5 translation; tense_choice 142+66=208, the tree's LAST zero-coverage leaf filled). Q1 RATIFIED: do NOT cross-credit congiuntivo (supplied-choice MCQ = SELECT the pairing, not FORM the subjunctive; congiuntivo.formation credit would be false; no hypothetical_protasis to mint on spec); condizionale.usage.hypothetical_apodosis is the honest 0.5 second bucket. The per-choice MCQ dead-tags stay INERT pending Housing's buildMcqResult choice-tag reader (known gap).

**Pluperfect tagging ACCEPTED** (v3): 16 avere_for_essere guards + tpp_es_mixed_01 retag (Analyst split: andate=mixed_gender_default, andato/andata generic). Design flag RULED for the author: the guard is the IMPERFECT avere (aveva andato), the clean avere_for_essere diagnostic (tense held constant); do NOT add a present-avere ("ha andato") guard - it double-counts with wrong_auxiliary_tense.

**Pluperfect cue-language = Rev 32 confirmed + sharpened:** a tense-naming FORMATION cue names the ENGLISH term (Italian optional), friendly-label vocabulary. FORMATION cues ONLY - usage/discrimination never name the mood/tense (leaks the answer). binds all-authors; the central grep-retrofit is task 7 (now RULED; ~118 cues). 
[discharged: TenseChoiceAuthor, 2026-07-21, periodo_ipotetico_delivery - 18+5 accepted, last zero-coverage leaf, congiuntivo cross-credit declined]
[discharged: PluperfectAuthor, 2026-07-21, misconception_tagging v3 - 16 avere_for_essere + andate retag accepted; imperfect-avere ratified]
[discharged: PluperfectAuthor, 2026-07-20, cue_tense_language - Rev 32 English-term cue rule ratified (formation only), retrofit = task 7]

## Task 5: person re-audit CLEAN; the mis-parse bug is already gone (2026-07-27, Architecture)

Re-audited the person field estate-wide against the ratified "produced person" convention. (a) person-set on MCQ items = 0 (convention holds - no incidental-person tags). (b) stored person vs id-encoded person across 598 id-tokenised items = 0 mismatches, so the "1sg leaf-token read as person" mis-parse (the ~12 congiuntivo items flagged 2026-07-21, e.g. the irregular_from_1sg leaf) is NOT present - already repaired (the Jul 22-23 congiuntivo reconciliation): cong_pres_1sg_*_3sg items correctly carry person 3sg (venga), not 1sg. 300 items carry person with no id person-token (person set from content); not cross-checkable without answer-morphology derivation (unreliable) and no signal they are wrong. No fix needed; closed as VERIFIED-CLEAN rather than fixed. 898 items carry person total.

## SpellingAuthor wave-1 ACCEPTED (2026-07-27, Architecture) - I was behind; picked up

SpellingAuthor's 141-MCQ wave-1 had been waiting on Architecture (missed while I worked an old snapshot). ACCEPTED: 9 leaves minted (orthography.spelling root + 8 class leaves: doubling/digraph/c_g_softening/qu_cu_cqu/silent_h/apostrophe_elision/capitalization/vowel_confusion) into data/buckets/orthography.json - the 141 items are no longer zero-coverage; 9 misconception specifics merged (registry now 106) + orthography.failed_elision widened to article-or-clitic; 4 glossary terms merged (doppie, digraph, elision, troncamento). FLAG 1 ruled KEEP orthography.spelling.<class> (no .italian; the accent sibling is the outlier, don't churn 141 items). FLAG 2: apostrophe boundary confirmed per Smith (author away; no de-dupe with AccentAuthor's apostrophe_not_accent). BUILD ASK routed to Housing: buildMcqResult must read choice_tags on a miss -> misconception_hit (unblocks per-choice diagnostics for ALL single-select MCQ - spelling, periodo ipotetico, gender). Wave-2 (typed doubling) GO. NB the SpellingAuthor seat + drill already existed - my earlier "spin out a new SpellingAuthor" was wrong.

## Owed-thread sweep (2026-07-27, Architecture): MCQ schema + Connective x2
**MCQ choice_tags RATIFIED** as the canonical MCQ-misconception schema (array parallel to choices; null | {class, misconception, proposed}). Housing's r68 reader is blessed; retro-activates SpellingAuthor's 141 tags + periodo ipotetico + gender drill. Optional per-tag `bucket` added for cross-topic distractors (default = item bucket); `class` is authoring metadata, `misconception` id is the recorded join key.
**Connective purpose ACCEPTED** (6+2, connective 60+24): Rev 27 dual-cite where subjunctive produced, single-markpoint where not, MCQ purpose-vs-cause; connective.purpose aggregate + connective.purpose.affinche_perche_subj leaf MINTED (chain order).
**Connective false-miss STAMPED**: 4 graded additions verified (poiche 0.8, in quanto 0.9, pero/ma 0.8, per 0.8), rails honoured.
[discharged: ConnectiveAuthor, 2026-07-27, purpose_connectives v2 - 6+2 accepted, 2 buckets minted]
[discharged: ConnectiveAuthor, 2026-07-27, false_miss_packet v2 - 4 graded additions verified]

## Owed-thread sweep cont. (2026-07-27): stress routings, mirror_of, paradigm decision, live-round-2, accent
- **Stress:** Vocab sidecar (don't fork their file) + Housing report-axes spec routed; Code fired by Smith. 
- **mirror_of RATIFIED** (over/under pairing field); the two grid questions ruled YES (event records chosen tense; verb_identity descriptor on family_verbs). Grids A6/A7 good to build.
...
## Task 8: vocab_help residue - infinitive-verb subset resolved; context-dependent remainder routed to Vocab (2026-07-27, Architecture)

Resolved the SAFE slice of the 88-entry residue centrally: infinitive multi_POS lemmas (candidate [noun,verb]) -> verb, on the rule that an infinitive help-hint is the verb (the substantivized-infinitive noun is rare and never a hint). Applied as a full-quoted-string id migration across the item files (vocabulary.it.<lemma>.translation -> vocabulary.it.<lemma>.verb.translation). The remainder (proper nouns, not_in_freq inflected forms needing lemmatisation, and multi_POS adjective/noun/adverb needing item context) is ROUTED to Vocab (Architecture_Vocab_vocab_help_residue) rather than auto-guessed - the residue file's own instruction ("do NOT auto-guess; a wrong POS pollutes the wrong bucket") is right, and Vocab owns the data + reads item context. residue file shrunk + annotated.

## LIVE: deployed translation mark still collapsed (0/3) - the v4-v11 panel isn't built + 2 new sharpenings (2026-07-27, Smith)

"Marco me vede al bar tutte le mattine" (me for mi) marked 0/3 with a "nearly correct" verdict and a 2.3/3 shown elsewhere. Faults: (1) header shows required-buckets-PASSED (0/3), must show X/N; (2) no greens - vede/al/tutte/le/bar/vedere earn nothing (cross-topic credit-everything not firing on the deployed path); (3) NEW: placement double-penalises the form slip - "me vede" is correctly PLACED (pre-verb), only the FORM is wrong, so placement = hit, form = miss (one error, one miss); (4) NEW: a Help lookup (mattina, declared unknown) is NEUTRAL, not a hard Wrong. Should read ~9/11. Items 1-2 are the already-specced v4-v11 build (deployed build is behind); 3-4 are new marking-contract rules. Routed crosstopic v12.
...
## Accent full regen over seed v1 EXECUTED (2026-08-03, seventh pass, Architecture)

306 items (276 + the seed-v1 carrier growth: meaning_pair 49, tense_bearing 14), generated by the
now-contract-consuming pipeline: placement-leaf wiring throughout (defects 1+3), both-verdict judgement
tags with zero nulls (defect 4; false_alarm rides the engine's flat-judgement path — no fifth bar),
misconception field canonical-or-absent (purity ruling; accent.* pseudo-ids eliminated at source).
Gate green on all five assertions; awaiting AccentAuthor acceptance. One mapping flagged for the seat:
inserted_as_accent -> wrong_kind.


## Rev 33 LANDED + re-paragraph pilot BUILT + over-reg bank resolved (2026-08-03, eighth pass)

**AUTHOR_BRIEF Revision 33 is live** (binds all-authors; the four staged items: lr-anchoring
exemption, common_errors specified, person field, item-shape law cross-ref). Seats pick it up via
class tokens on wake; BRIEF_REV33_STAGED.md is now the landed record.

**Re-paragraph pilot built** (pilot_reparagraph.html): 10 longest single-block explanations, one per
topic, paragraphing byte-identical-wording; two labelled bullet variants. Renderer dependency found
and routed (R8: textContent collapses newlines; white-space: pre-line one-liner to Housing). Sweep
scope derived: 239 single-block explanations >420 chars. AWAITING Smith's bless; no sweep before it —
"a mass editorial rewrite without an approved sample is how house style drifts."

**Over-reg bank: no disk source exists.** Task-11's "over-reg distractor bank ingest" traced:
