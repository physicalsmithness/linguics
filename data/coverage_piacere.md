...
1. **Every markpoint is word-anchored (`match_at: "word"`), per criterion 18.** The trigger case is `piace`, a prefix-substring of the cue word `piacere`: unanchored, a bare-infinitive answer would be credited for `piace`, and a negative guard cannot override a positive match. The same rightward risk runs through the batch (`mi piace` inside `mi piaceva`, `ti piace` inside `ti piaceva`) and the leftward risk through the short clitics and a-phrases (`le` inside `delle`, `a marco` inside `da marco`). All 43 markpoints now carry `match_at: "word"`, matching the existential batch's reconciliation. This supersedes the `match_at: "end"` the first draft used.
...
6. **`must_not_include` is plain strings only.** The dispatch worked example put explanatory notes inside `must_not_include` objects; the shipped engine uses plain strings on all live entries and checks them after `any_phrases` anyway, so notes there would be inert. The "liked thing is the subject" teaching therefore lives in the explanations (glossary-wrapped) and examiner_notes.
...
- Word-anchoring (`match_at: "word"`) is now applied to all 43 markpoints per criterion 18 (engine-supported since 2026-07-14-r4); no item-rework awkwardness remained after the past-auxiliary change.
