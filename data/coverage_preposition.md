...
## Ninth touch, 2026-08-03 (articulated paradigm fill: 22 items, 24 cells)

Commission: Architecture_PrepositionAuthor_articulated_empty_cells (24 of 35 cells untested once the paradigm bands landed). Delivered prep_af_12..33: six fragments on the rare cells (dello, dell', nello, sullo, sugli, sulle), fourteen in-context singles (nel, alla, della, degli, dei, ai, all', dallo, dagli, dalla, dall', nell', nelle, sui), and two natural pairs carrying the four remaining cells at two markpoints each: dal 3 al 10 agosto (dal + al, the commission's top priority al in its natural date-range home) and dalle 9 alle 18 (dalle + alle, opening hours). 22 items over the guessed 15-20 because the rare cells were cheapest as 1:1 fragments.

Brief compliance at Rev 33: nouns level-checked against each item's CEFR (Rev 25); cues name trigger + context only (Rev 26); target forced by construction (Rev 28 clean-by-shape for contraction drills); no-change-forms boundary satisfied — the blank's answer differs from everything cited (Rev 33); wrong_answer_is_form_error_only: true on all 22 (operand fully supplied, so a wrong answer is a mis-formation). Pairs word-anchored both directions; af_33's examiner_note records why (unanchored 'alle 9' would sit inside the correct 'dalle 9'). Validations: zero blank-boundary spans, zero cross-containment, zero accents, primary any_phrases lead with the contracted form so slot derives per markpoint. prep_af_11 untouched and slotless per the commission.
