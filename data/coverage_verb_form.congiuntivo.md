...
1. **debba / deva (dovere singular subjunctive).** Item `cong_pres_1sg_dovere_3sg_01` accepts `debba` at full credit and `deva` as a graded `any_phrases` object at 0.9 with a register note. Both are fully standard; debba is the more usual and the careful-writing default, deva is common in speech. **Ruling wanted:** is the 1.0/0.9 split house policy, or should both be 1.0 (they are equally grammatical), or should deva be excluded as register-marked? This mirrors the present-indicative chat's debbo/devo flag, so a single consistent policy across the two trees would be ideal.
