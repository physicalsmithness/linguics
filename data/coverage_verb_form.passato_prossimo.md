...
**Agreement-bucket central consolidation + 11 person backfills (2026-08-12)**: Architecture ran a central retrofit on my file on 2026-08-03 — 16 markpoint re-points collapsing the 8 old leaf-buckets (`with_essere.{msg,fsg,mpl,fpl}` and `with_avere.preceding_dop_{lo,la,li,le}`) into two parent buckets (`with_essere` and `with_avere.preceding_dop`) while keeping specific-shape buckets separate (`mixed_gender_default`, `preceding_partitive_ne`, `no_preceding_dop`). Subtopic strings preserve the leaf name for breadcrumb; markpoint `bucket` is the parent for scoring. any_phrases converted to dict form with `match_at: "word"` universally on the agreement axis. Verified from disk 2026-08-12. **11 person backfills** applied on items the central aux-scan couldn't pin: 7 × 3pl mixed-gender/plural essere frames (pp_agr_ess_mpl_01, pp_agr_ess_fpl_02, pp_agr_ess_mix_01..05), 2 × 1sg preceding-DOP-la Q&A (pp_agr_av_dop_la_01, _02), 2 × 1sg adverb-placement pinned by the "masc sg" parenthetical (pp_adv_pos_03, _05). Estate-wide person completeness on PP: 125/135 with 1sg..3pl, 10 null on form-only fragments, 0 missing.
