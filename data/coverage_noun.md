...
## Wake pass, 2026-08-14 (tier-2 search return; answer-leak fix; taxonomy weigh-in)

**Tier-2 SEARCH returned** (`review_packets_tier2/returned/SEARCH_noun_2026-08-13_returned.json`): 648 findings that the 913 estate reference translations demonstrate a noun bucket, mined from the 665 Italian references (the 248 it_en items skipped — learner produces English). A vocab-validated detector (pos=noun lexicon rejects clitic+verb, possessive, and wrong-POS homographs; article-as-gender-oracle; determiner/preposition-gated plurals) curated across five passes; blind 30-row audit returned 0 false positives. Claim policy (the packet's open question, my call): leaf + direct skill-aggregate (noun.gender / noun.plural), NOT the root `noun` container — per Rev 27 + coverage-doesn't-walk-ancestors, siding with kept-parents over struck, but stopping below the pure container.

**noun_gen_e_09 answer-leak fixed** (answer_leak_dispatch): the answer `il` recurred downstream in "il denaro che si investe"; carrier rephrased to "In economia, ____ capitale è fondamentale per crescere" (no downstream article), marking and candidate_forms intact.

**Noun-class taxonomy weigh-in** (Architecture_Vocab_noun_class_taxonomy): mano must leave the proposed shortened_o_fem (not clipped; plural mani is regular-irregular, not invariable); brindisi (m) breaks invariable_i_fem's feminine-only; the set still lacks a meaning_split class and a common-gender class broader than -ista; via is a legitimate feminine noun, not only the POS-bug residue. Number-axis (invariability as a number property) endorsed.
