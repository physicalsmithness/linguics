...
1. **si vende libri (agreement slip): RULED 0.5 and APPLIED.** I opened this as flat-vs-graded rather than set graded-credit policy from an author seat; Architecture ruled parity with the a-me-mi-piace / ha-detto-che-veniva class (v2, 2026-07-15) and I applied it (v3, 2026-07-18). The singular slip is now a graded `any_phrases` entry at credit 0.5 with a per-item register note on all nine items (six agreement: si_pass_agr_01/03/04/05/07/08; three discrimination: si_vs_02/04/06). The thirteen reverse over-corrections stay flat, as ruled. Marker-verified (22-case engine-exact harness): slip 0.5, correct 1.0, ordering safe, the 0.9 vendono dodge on agr_01 preserved. Register wording is interim pending Architecture (thread v3). Thread: inter_chat/Architecture_SiConstructionsAuthor_graded_credit_parity.md (v3, APPLIED).
