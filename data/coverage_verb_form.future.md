...
- **The 1sg/3sg written accents** (parlerò, lavorerà, avrò, sarà, darà, …) are treated as spelling-meaning errors throughout: the accented form is accepted, the bare form (parlero, sara) rejected in `must_not_include`. This relies on the marker preserving diacritics (the same basis as the present tree's è/e and può/puo guards); if that ever changes, every accent guard in this batch would need revisiting.
...
1. **Accent guards depend on diacritic-preserving normalisation.** Every 1sg/3sg item accepts the accented form (parlerò, sarà) and rejects the bare form (parlero, sara) via `must_not_include`. This is sound *only if* the marker preserves accents when matching, which is the same assumption the present and imperfect trees already ship on (è vs e, può vs puo). No action needed unless that normalisation policy is under review; flagging because this batch leans on it heavily (16 items).
