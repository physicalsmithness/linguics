...
- **Architect / housing:** as the dispatch noted, `verb_form.gerundio` is not yet in the housing's `VERBS_CURRICULUM_ORDER`, so it will currently sort to the end of the Verbs group. It is non-finite, so it probably slots after the finite tenses (and after, or alongside, the point where the progressive's prerequisites — present and imperfect of stare — are in place). Flagging for the architect to route to housing; not actioned here.
