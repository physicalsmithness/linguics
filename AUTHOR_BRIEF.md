...
20. **Cue by meaning, not by fragment (Rev 22).** Where a production item would cue its target with a bare Italian answer-fragment that (a) reads as the whole answer or (b) presupposes the very form the item tests, cue it instead with a bracketed English target pinning meaning and register ("Questo regalo è ___ [for you, informal]" → per te), and set `prompt_supplies_base_form: false` so the cue-misread second-chance rescue applies. This does NOT touch the legitimate citation-form trigger: an infinitive to conjugate (studiare), a base adjective to agree (rosso → rossa), a supplied lemma to inflect — those name the honest operand and stay. The test has two parts. (1) Is the cue the operand the learner TRANSFORMS, or a slice of the answer that HIDES the operation? (2) **The sharper predictor (Rev 23, from ComparisonAuthor): is the fragment-only answer visibly BROKEN or plausibly FLUENT?** Typing just the cue `alto` yields "Marco è alto Luca" - self-evidently wrong, so the learner is rescued by the sentence itself and a bare cue is defensible. Typing just the cue `agire` yields "È più facile criticare agire" - fluent-looking and merely missing its connector, so the learner is TRAPPED, takes an unrecognised miss, and is diagnosed as failing the grammar when they misread the cue. Gloss the trapping ones. It is not fragment-ness that harms; it is whether the wrong answer looks wrong. Triggered by prep_sf_03 (bare (te) both under-specified the per and presupposed the stressed pronoun; Smith produced tuo and, because prompt_supplies_base_form was wrongly true, got no second chance). Next-touch: PrepositionAuthor (55 items) and ComparisonAuthor (19) hold the candidate class; new items follow immediately; the criterion-17 sweep flags stragglers estate-wide. (3) **Ambiguity condition (Rev 29):** gloss only where the English is ambiguous across the candidate set. Where the gloss is a one-to-one translation of the answer (all -mente derivation, any single-lexeme production item), do NOT gloss — widen the gloss until it spans the candidate set (best; can bait the miss), else change the instrument to MCQ, else vocab_help the citation form. As written, Rev 22/25's 'gloss the target' would destroy every derivation item with an above-level cue. (4) **Invariable-operand (Rev 30):** the exemption reaches only an operand the learner transforms; an INVARIABLE word (cio, qualcuno, qualcosa, chiunque, qualunque, qualsiasi, niente, nulla, ogni, qualche) has nothing to inflect, so the exemption lapses and Rev 23's broken-vs-fluent test decides - gloss the fluent bare-cue cases, or reframe with register stated where the invariable is the genuine formal target. (5) **Supplied-choice exemption (Rev 31):** criteria 13 and 20 do not apply to supplied-choice/selection items (both forms given, no chip, no misreadable fragment), mirroring criterion 19's Rev 21(i).

21. **Formation forces its target (Rev 28).** Every formation item must force its target tense/mood by one of: (a) the tense/mood named in the cue chip — licensed by Rev 26, it is the TRIGGER, not a criterion-13 leak: `(credere, passato prossimo)`; (b) a supplied partial form (`io ho ____`, `sto ____`); (c) a genuinely EXCLUDING frame (co-text same-tense verb, telic bound 'in due giorni', se + congiuntivo protasis) — a bare temporal adverb is NOT a trigger, because ieri/domani/quella sera leave the aspect, register or tense choice open; (d) a supplied-choice menu; (e) imperativo's command frame. Test: write the best competing tense into the blank — if the sentence is well-formed and contextually plausible, the item is AMBIGUOUS and a learner producing it records formation misses they never attempted, which is a mastery mis-attribution (the tense CHOICE is tense_choice's to test, with candidates supplied). An ambiguous frame also forbids `prompt_supplies_base_form: true` (that flag claims a wrong answer can only be a form error — false when a tense choice is open). The clean-topic patterns to copy: congiuntivo's governing trigger, gerundio's named target, imperativo's command frame.
...
|---|---|---|---|
| 13 | all-authors | self-audit for rule-naming chip texts | **DISCHARGED CENTRALLY 2026-07-17** — Architecture ran it across all 1,144 chips / 29 topics rather than asking 25 seats. 17 raw hits, 12 false positives (trigger + context, both required elsewhere); real exposure **5 chips, all in pronoun**, routed. `[discharged: all-authors except PronounAuthor, 2026-07-17, central chip audit]` Test sharpened at Rev 26. |
...
| 21 | all-authors | 142 ambiguous formation items, itemised per topic in AUDIT_formation_trigger_2026-07-19.md | **DISPATCHED 2026-07-19 (Smith ruled B: per-author rework)** — 7 work-order threads (Future 45, PassatoRemoto 35, Passato 22, ConditionalFormation 19, Imperfect 11, PresentFormation 7, Pluperfect 3); stamps accumulate per seat |
